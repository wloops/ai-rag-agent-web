...
const formatDuration = (value: number | null) =>
  value === null || Number.isNaN(value) ? "--" : `${value} ms`;
...
export default function ChatSessionPage() {
  ...
  const renderPreviewPanel = () => {
    ...
    if (!citationPreview) return <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-4 py-6 text-sm text-slate-500">选择引用或召回片段后，这里会展示对应 chunk 的原文预览。</div>;
    return (
  ...
  const renderDebugPanel = () => {
    if (!debugState) return <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-4 py-6 text-sm text-slate-500">还没有可展示的调试数据。发送一条新问题后，这里会显示本次问答的阈值判断、上下文预览、耗时和召回片段。</div>;
    return (
      <>
        <div className="space-y-3">
          <h4 className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-slate-500"><Search className="h-3.5 w-3.5" />请求概览</h4>
          <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
            <div><div className="mb-0.5 text-[10px] text-slate-400">问题</div><div className="rounded border border-slate-100 bg-slate-50 p-1.5 text-xs font-medium text-slate-800">{debugState.question}</div></div>
            ...
            <span className={clsx("inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-[11px] font-medium", debugState.decision === "reject" ? "border-amber-200 bg-amber-50 text-amber-700" : "border-emerald-200 bg-emerald-50 text-emerald-700")}>
        ...
        <div className="space-y-3">
          <h4 className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-slate-500"><Clock3 className="h-3.5 w-3.5" />耗时拆分</h4>
        ...
        <div className="space-y-3">
          <h4 className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-slate-500"><TextQuote className="h-3.5 w-3.5" />最终上下文</h4>
          <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
            {debugState.finalContextPreview ? (
              <>
                ...
                {debugState.finalContextPreview.length > 280 ? <button onClick={() => setIsContextExpanded((current) => !current)} className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-700">{isContextExpanded ? <>收起<ChevronUp className="h-3.5 w-3.5" /></> : <>展开<ChevronDown className="h-3.5 w-3.5" /></>}</button> : null}
              </>
        ...
        <div className="space-y-3">
          <h4 className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-slate-500"><Zap className="h-3.5 w-3.5" />召回片段 ({debugState.retrievedChunks.length})</h4>
          <div className="space-y-2">
            {debugState.retrievedChunks.map((item, index) => {
              const previewTarget: PreviewableChunk = { chunk_id: item.chunkId, document_id: item.documentId, filename: item.filename, chunk_index: item.chunkIndex, start_offset: item.startOffset, end_offset: item.endOffset, snippet: item.snippet };
              const isCurrentPreview = isCitationPreviewOpen && selectedPreviewSource?.document_id === item.documentId && selectedPreviewSource?.chunk_id === item.chunkId && selectedPreviewSource?.chunk_index === item.chunkIndex;
              return (
                <div key={`${item.chunkId}-${index}`} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                  <div className="mb-2 flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-400">
                        <span>chunk {item.chunkIndex}</span><span>/</span><span>id {item.chunkId}</span>
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3 border-t border-slate-100 pt-2">
                    ...
                    <button onClick={() => handleOpenChunkPreview(previewTarget)} className={clsx("rounded-md px-2 py-1 text-xs font-medium transition-colors", isCurrentPreview ? "bg-blue-50 text-blue-700" : "text-blue-600 hover:bg-blue-50 hover:text-blue-700")}>查看原文</button>
                  </div>
                </div>
              );
            })}
      ...
      <div className="relative flex min-w-0 flex-1 flex-col bg-white">
        <div className="z-10 flex h-14 items-center justify-between border-b border-slate-200 bg-white/95 px-6 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-medium text-slate-500"><Database className="h-3 w-3" />{knowledgeBase?.name ?? "未匹配知识库"}</div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setActiveRightPanel((current) => (current === "debug" ? null : "debug"))} className={clsx("flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors", isDebugOpen ? "border-blue-200 bg-blue-50 text-blue-700" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50")}><Terminal className="h-3.5 w-3.5" />调试面板</button>
            <button className="rounded-md p-1.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"><MoreHorizontal className="h-4 w-4" /></button>
          </div>
        </div>
                      ...
                      {message.citations.length > 0 ? (
                        <div className="border-t border-slate-100 bg-slate-50/50 px-5 pb-5 pt-2">
                          <button onClick={() => setExpandedCitationGroups((current) => ({ ...current, [message.id]: !current[message.id] }))} className="mb-3 mt-2 flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-left transition-colors hover:border-blue-200 hover:bg-blue-50/60">
                            <div className="flex items-center gap-2"><Layers className="h-4 w-4 text-slate-400" /><span className="text-xs font-semibold uppercase tracking-wider text-slate-500">引用来源 ({message.citations.length})</span></div>
                            <div className="flex items-center gap-2 text-[11px] text-slate-400"><span>{isCitationGroupExpanded ? "收起" : "展开查看"}</span>{isCitationGroupExpanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</div>
                          </button>
                          {isCitationGroupExpanded ? <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{message.citations.map((citation) => {
                            const isPreviewAvailable = Boolean(citation.chunk_id);
                            const isCurrentPreview = isCitationPreviewOpen && selectedPreviewSource?.document_id === citation.document_id && selectedPreviewSource?.chunk_id === citation.chunk_id && selectedPreviewSource?.chunk_index === citation.chunk_index;
                            return <div key={`${message.id}-${citation.document_id}-${citation.chunk_id ?? citation.chunk_index}`} className="group flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-3 transition-all hover:border-blue-300 hover:shadow-sm">
                              <div className="flex items-start justify-between gap-2"><div className="flex min-w-0 items-center gap-2"><FileText className="h-4 w-4 flex-shrink-0 text-blue-500" /><span className="truncate text-xs font-medium text-slate-700 transition-colors group-hover:text-blue-700">{citation.filename}</span></div><span className="flex-shrink-0 rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] text-slate-500">chunk {citation.chunk_index}</span></div>
                              <p className="line-clamp-2 text-xs leading-relaxed text-slate-500">{citation.snippet ?? "暂无摘要"}</p>
                              <div className="flex items-center justify-between gap-3 border-t border-slate-100 pt-2"><div className="text-[10px] text-slate-400">{citation.start_offset !== null && citation.end_offset !== null ? `offset ${citation.start_offset} - ${citation.end_offset}` : "未提供定位信息"}</div><button onClick={() => handleOpenChunkPreview({ ...citation, snippet: citation.snippet ?? null })} disabled={!isPreviewAvailable} className={clsx("rounded-md px-2 py-1 text-xs font-medium transition-colors", isPreviewAvailable ? (isCurrentPreview ? "bg-blue-50 text-blue-700" : "text-blue-600 hover:bg-blue-50 hover:text-blue-700") : "cursor-not-allowed text-slate-300")}>{isPreviewAvailable ? "查看原文" : "原文不可用"}</button></div>
                            </div>;
                          })}</div> : null}
                        </div>
                      ) : null}
        ...
        <div className="border-t border-slate-100 bg-white p-4">
          <div className="mx-auto max-w-4xl rounded-2xl border border-slate-300 bg-white shadow-sm transition-all focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-500/10">
            <textarea value={input} onChange={(event) => setInput(event.target.value)} className="min-h-[80px] w-full resize-none bg-transparent p-4 text-sm text-slate-900 outline-none placeholder:text-slate-400" placeholder="继续输入问题，系统将基于当前知识库继续检索回答..." />
            <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/50 px-3 py-2">
              <div className="flex items-center gap-1.5 rounded-lg p-1.5 text-xs font-medium text-slate-500"><Database className="h-4 w-4" /><span className="hidden sm:inline">{knowledgeBase?.name ?? "未匹配知识库"}</span></div>
              <button onClick={handleSend} disabled={!input.trim() || isSending || !conversation} className={clsx("flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium shadow-sm transition-all", input.trim() && !isSending && conversation ? "bg-blue-600 text-white hover:bg-blue-700" : "cursor-not-allowed bg-slate-200 text-slate-400")}>{isSending ? "发送中" : "发送"}<Send className="h-3.5 w-3.5" /></button>
            </div>
          </div>
        </div>
      </div>
      ...
      {activeRightPanel ? (
        <div className="z-20 flex w-80 flex-shrink-0 flex-col border-l border-slate-200 bg-slate-50 shadow-[-4px_0_24px_-12px_rgba(0,0,0,0.05)]">
          <div className="flex h-14 items-center justify-between border-b border-slate-200 bg-white px-4">
            <div className="flex items-center gap-2 text-sm font-medium text-slate-800">{isDebugOpen ? <Terminal className="h-4 w-4 text-blue-600" /> : <FileText className="h-4 w-4 text-blue-600" />}{isDebugOpen ? "RAG 调试视图" : "原文预览"}</div>
            <button onClick={() => setActiveRightPanel(null)} className="rounded-md p-1.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"><X className="h-4 w-4" /></button>
          </div>
...
function Metric({ label, value, truncate = false }: { label: string; value: string; truncate?: boolean }) {
  return <div><div className="mb-0.5 text-[10px] text-slate-400">{label}</div><div className={clsx("text-slate-700", truncate && "truncate")} title={truncate ? value : undefined}>{value}</div></div>;
}

function MetricCard({ label, value }: { label: string; value: string }) {
  return <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm"><div className="mb-1 text-[10px] uppercase tracking-wider text-slate-400">{label}</div><div className="text-sm font-medium text-slate-800">{value}</div></div>;
}

  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <div className="rounded-2xl border border-slate-200 bg-white px-6 py-4 text-sm text-slate-500 shadow-sm">正在检查登录状态...</div>
      </div>
    )
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-12">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-5 text-2xl font-bold tracking-tight text-slate-900">连接你的知识库系统</h1>
          <p className="mt-2 text-sm text-slate-500">使用真实账号登录或注册后，即可开始知识库管理与 RAG 问答。</p>
        </div>
        ...
        <div className="mb-6 grid grid-cols-2 rounded-2xl bg-slate-100 p-1 text-sm">
          <button
            ...
            className={`rounded-xl px-3 py-2 font-medium transition-colors ${mode === 'login' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
          >
            登录
          </button>
          <button
            ...
            className={`rounded-xl px-3 py-2 font-medium transition-colors ${mode === 'register' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
          >
            注册
          </button>
        </div>
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          >
            {isSubmitting ? (mode === 'login' ? '登录中...' : '注册中...') : mode === 'login' ? '登录' : '注册并登录'}
